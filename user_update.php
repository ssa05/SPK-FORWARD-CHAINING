...
if (isset($_POST['save'])) {
    ...
    if (empty($error)) {
        if ($action == 'add') {
            if (mysqli_num_rows(mysqli_query($con, "select * from user where username='" . $username . "'")) > 0) {
                ...
                $q = "insert into user(nama_lengkap,username,password,level) values ('" . $nama_lengkap . "','" . $username . "','" . $password . "','" . $level . "')";
                mysqli_query($con, $q);
                exit("<script>location.href='" . $link_data . "';</script>");
            }
        }
        if ($action == 'edit') {
            $q = mysqli_query($con, "select * from user where id_pengguna='" . $id . "'");
            ...
            if (mysqli_num_rows(mysqli_query($con, "select * from user where username='" . $username . "' and username<>'" . $username_tmp . "'")) > 0) {
                ...
                $q = "update user set nama_lengkap='" . $nama_lengkap . "',username='" . $username . "' where id_pengguna='" . $id . "'";
                mysqli_query($con, $q);
                exit("<script>location.href='" . $link_data . "';</script>");
            }
        }
    ...
    if ($action == 'delete') {
        $id = $_GET['id'];
        mysqli_query($con, "delete from user where id_pengguna='" . $id . "'");
        exit("<script>location.href='" . $link_data . "';</script>");
    }
}
?>
