...
if (isset($_POST['save'])) {
    ...
    if (empty($error)) {
        if ($action == 'add') {
            if (mysqli_num_rows(mysqli_query($con, "select * from magang where kode_perusahaan='" . $kode_perusahaan . "'")) > 0) {
                $error = 'kode_perusahaan sudah ada';
            } else {
        ...
        if ($action == 'edit') {
            ...
            $kode_perusahaan_tmp = $r['kode_perusahaan'];
            if (mysqli_num_rows(mysqli_query($con, "select * from magang where kode_perusahaan='" . $kode_perusahaan . "' AND kode_perusahaan<>'" . $kode_perusahaan_tmp . "'")) > 0) {
                $error = 'kode_perusahaan sudah ada';
            } else {
                $q = "update magang set kode_perusahaan='" . $kode_perusahaan . "',nama_perusahaan='" . $nama_perusahaan . "',posisi_magang='" . $posisi_magang . "',keterangan='" . $keterangan . "' where id_perusahaan='" . $id . "'";
                mysqli_query($con, $q);
                exit("<script>location.href='" . $link_data . "';</script>");
            }
        }
